...
// Initially empty; will clearly not pass correctness
  ...
  if((row<Ni)&&(col<Nj)){
    ...
    for(int k=0;k<Nk;k++){
      //C[i][j] += A[k][i]*B[k][j];
      temp += A[k*Ni+row]*B[k*Nj+col];
    }
    C[row*Nj+col] = temp;
  }
}
__global__ void atbUnrollk(const double *A, const double *B, double *C, int Ni, int Nj, int Nk) {
// Initially empty; will clearly not pass correctness
  int col = blockIdx.y*blockDim.y+threadIdx.y; // j
  int row = blockIdx.x*blockDim.x+threadIdx.x; // i
  ...
  if((row<Ni)&&(col<Nj)){
    ...
    for(int k=rem;k<Nk;k+=2){
      //C[i][j] += A[k][i]*B[k][j];
      temp += A[k*Ni+row]*B[k*Nj+col];
      temp += A[(k+1)*Ni+row]*B[(k+1)*Nj+col];
    }
    C[row*Nj+col] = temp;
  }
}

__global__ void atbUnrollj(const double *A, const double *B, double *C, int Ni, int Nj, int Nk) {
// Initially empty; will clearly not pass correctness
...
__global__ void atbUnrolli(const double *A, const double *B, double *C, int Ni, int Nj, int Nk) {
// Initially empty; will clearly not pass correctness
  int col = blockIdx.y*blockDim.y+threadIdx.y; // j
  ...
  if((row<Ni)&&(col<Nj)){
    ...
    for(int k=0;k<Nk;k++){
      //C[i][j] += A[k][i]*B[k][j];
      temp += A[k*Ni+row]*B[k*Nj+col];
      temp2 += A[k*Ni+row+blockDim.x]*B[k*Nj+col];
    }
    C[row*Nj+col] = temp;
    C[(row+blockDim.x)*Nj+col] = temp2;
  }
}
__global__ void atbUnrolla(const double *A, const double *B, double *C, int Ni, int Nj, int Nk) {
// Initially empty; will clearly not pass correctness
  int col = (blockIdx.y*blockDim.y+threadIdx.y)*2; // j
  int row = blockIdx.x*blockDim.x*2+threadIdx.x; // i
  ...
  if((row<Ni)&&(col<Nj)){
    ...
    for(int k=0;k<rem;k++){
      temp += A[k*Ni+row]*B[k*Nj+col];
      temp1 += A[k*Ni+row+blockDim.x]*B[k*Nj+col];
      temp2 += A[k*Ni+row]*B[k*Nj+col+1];
      temp3 += A[k*Ni+row+blockDim.x]*B[k*Nj+col+1];
    }
...
while(1){
  ...
  checkCUDAError("cudaMemcpy H2D failure");
    ...
    // Launch kernel
    if(unrolling=='i'){
      atbUnrolli<<<grid, block>>>(d_A, d_B, d_C, Ni,Nj,Nk);
    }
    else if(unrolling=='j'){
      atbUnrollj<<<grid, block>>>(d_A, d_B, d_C, Ni,Nj,Nk);
    }
    else if(unrolling=='k'){
      atbUnrollk<<<grid, block>>>(d_A, d_B, d_C, Ni,Nj,Nk);
    }
    else if(unrolling=='a'){
      atbUnrolla<<<grid, block>>>(d_A, d_B, d_C, Ni,Nj,Nk);
    } 
    else{
